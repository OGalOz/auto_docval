...
def get_func_name_to_vars_from_file(python_file_fp):
    ...
    *DOCDONE
    ...
    """
    function_names_to_starts_and_ends = get_function_names_starts_and_ends(python_file_fp)
    #print(function_names_to_starts_and_ends)
    funcN2vars = get_func_name_to_variables(function_names_to_starts_and_ends, 
                                                        python_file_fp)
    return funcN2vars
...
def create_documentation_args_returns_str(funcN2vars,
    ...
    *DOCDONE
    Args:
        funcN2vars (dict): Dict mapping function name to a list of variables in def
            func_name -> list<var>
        ...
        types_cfg_json_fp (string): Path to all type spec file.
            Restriction: is_file=1
    Notes:
        Every object in type_spec_d should have 'subtype' in it,
        as well as the 'desc' key
    Returns:
        funcN2vars2docstr (dict): maps func names to dict with 
                                  vars pointing to their docstrs_major_l
    """

    funcN2vars2docstr_l = {}
    for func_name, func_vars in funcN2vars.items():
        var2docstr_l = {}
        for var in func_vars:
            if var not in type_spec_d:
                raise Exception(f"Variable {var} from function {func_name} " + \
                                f"not in type_spec_d from file {types_cfg_json_fp}")
        ...
        funcN2vars2docstr_l[func_name] = var2docstr_l
    ...
    *DOCDONE
    Args:
        ...
        type_spec_d (dict): The dict that holds the information about all the variables.
            var -> spec_d
                var (string): Name of a variable
                var -> spec_d (dict), The dict that holds the information about a single variable.
    ...
    if max_num_layers != -1 and current_num_layer >= max_num_layers:
        return ""
    if var in ["string", "int", "float", "skip", "bool"]:
        return ""
    ...
    if dict_key is not None:
        this_var_doc_str = f"{dict_key} -> {var} ({crt_spec_d['subtype']}), {crt_spec_d['desc']}"
    else:
        this_var_doc_str = f"{var} ({crt_spec_d['subtype']}): {crt_spec_d['desc']}"
...
def get_func_name_to_variables(func_names_to_starts_and_ends, python_file_fp):
    """
    *DOCDONE
    Args:
        func_names_to_starts_and_ends (dict): Dict mapping function names to definition's start and end line within file
    ...
    funcN2vars = {}
    for func_name in func_names_to_starts_and_ends.keys():
        f_strt = func_names_to_starts_and_ends[func_name]["func_start"]
        f_end =  func_names_to_starts_and_ends[func_name]["func_end"]
        ...
        if '' in func_vars:
            func_vars.remove('')
        if len(func_vars) > 0:
            funcN2vars[func_name] = func_vars
    ...
    *DOCDONE
    ...
    for i in range(len(file_lines)):
        ...
        if i in end_to_func_name:
            ...
            for var, var_docstrs_l in vars_d.items():
